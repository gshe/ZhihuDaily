...
@property(nonatomic, assign, getter=isDragging)
    BOOL dragging; // 是否在用手指拖动scrollView

@property(nonatomic, strong, readonly) UIScrollView *scrollView;
@property(nonatomic, strong, readonly) UIPageControl *pageControl;

@property(nonatomic, copy) int (^numberOfPagesInAutoLoopView)
    (WFAutoLoopView *autoLoopView);
...
- (void)addPageControl {
  ...
  _pageControl.autoresizingMask = UIViewAutoresizingFlexibleLeftMargin |
...
- (void)reloadCells {
  ...
  for (NSInteger i = 0; i < _cells.count; i++) {
    ...
    CGRect cellFrame = CGRectMake(kScreenWidth * i, 0, kScreenWidth,
...
- (void)autoLoopScrollCell {
  ...
  [NSObject
      cancelPreviousPerformRequestsWithTarget:self
                                     selector:@selector(autoLoopScrollCell)
                                       object:nil];
  CGPoint newOffset = CGPointMake(CGRectGetWidth(_scrollView.bounds) * 2,
                                  _scrollView.contentOffset.y);
  [_scrollView setContentOffset:newOffset animated:YES];
  [self performSelector:@selector(autoLoopScrollCell)
             withObject:nil
             afterDelay:_autoLoopScrollInterval];
}
...
- (void)scrollViewDidEndDecelerating:(UIScrollView *)scrollView {
  [scrollView setContentOffset:CGPointMake(CGRectGetWidth(scrollView.bounds), 0)
                      animated:YES];
}
...
- (void)setBanners:(NSArray *)banners {
  ...
  self.autoLoopViewCellForIndex = ^UIView *(int index) { return cells[index]; };
...
- (void)setNumberOfPagesInAutoLoopView:
        (int (^)(WFAutoLoopView *))numberOfPagesInAutoLoopView {
  ...
  _pageControl.frame = CGRectMake((self.bounds.size.width - pcSize.width) * 0.5,
                                  self.bounds.size.height - pcSize.height,
                                  pcSize.width, pcSize.height);
}
