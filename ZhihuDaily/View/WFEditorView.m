...
- (void)configUI {
  ...
  UILabel *titleLabel =
      [[UILabel alloc] initWithFrame:CGRectMake(12, 0, 40, 40)];
  titleLabel.text = @"主编";
  ...
  UIImageView *arrowsImage = [[UIImageView alloc]
      initWithFrame:CGRectMake(kScreenWidth - 35, 12, 15, 15)];
  arrowsImage.image = Image(@"leftEnter.png");
  [self addSubview:arrowsImage];

  UIView *bottomLine =
      [[UIView alloc] initWithFrame:CGRectMake(0, 39.5, kScreenWidth, 0.5)];
...
- (void)setAvatarImageArr:(NSArray *)avatarImageArr {
  ...
  for (EditorDataModel *editor in avatarImageArr) {
    ...
    [avatarImg sd_setImageWithURL:[NSURL URLWithString:editor.avatar]
